...
export function HotelHero({ isEnglish }: { isEnglish: boolean }) {
  return (
    <AnimatedSection className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20" style={{ backgroundImage: "radial-gradient(circle at 50% 50%, rgba(120, 119, 198, 0.3) 0%, transparent 50%)" }}></div>
      <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
        <h1 className="text-5xl md:text-7xl font-bold text-white tracking-tight drop-shadow-lg">{isEnglish ? "Hospitality Excellence" : "Ταξιδιωτική Τελειότητα"}</h1>
        <p className="text-xl md:text-2xl text-white/95 max-w-3xl mx-auto drop-shadow-lg font-light mt-6">{isEnglish ? "Complete hotel and restaurant renovation services. From concept to opening, we create unforgettable guest experiences." : "Ολοκληρωμένες υπηρεσίες ανακαίνισης ξενοδοχείων. Από τον σχεδιασμό έως το άνοιγμα, δημιουργούμε αξέχαστες εμπειρίες."}</p>
      </div>
    </AnimatedSection>
  )
}
